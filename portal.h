...
<div class="oled-info-row" style="flex-direction:column; align-items:flex-start; gap:8px; border-bottom:none; margin-top:8px;">
  <span class="oled-info-lbl" style="color:var(--txt)">Unsupported Characters</span>
  <div id="unsupportedCharsWrap" style="font-size:0.85em; color:var(--txt2); width:100%; border:1px solid var(--brd); border-radius:4px; padding:6px; background:var(--bg2);">
    <div id="unsupportedCharsList" style="display:flex; flex-direction:column; gap:4px; max-height:100px; overflow-y:auto; padding-right:4px;">No unsupported characters.</div>
  </div>
</div>
</div>
</div>
...
function bindEdWheelForward(el){
el.addEventListener('wheel',function(e){
if(!curFile)return;
e.preventDefault();
edArea.scrollTop+=e.deltaY;
syncEdScroll();
},{passive:false});
}

var edTouchY=0;
function bindEdTouchForward(el){
el.addEventListener('touchstart',function(e){
if(e.touches&&e.touches.length){edTouchY=e.touches[0].clientY}
},{passive:true});
el.addEventListener('touchmove',function(e){
if(!curFile||!e.touches||!e.touches.length)return;
...
edArea.addEventListener('scroll',syncEdScroll);
...
bindEdTouchForward(edGutter);
bindEdTouchForward(document.getElementById('lineNums'));
